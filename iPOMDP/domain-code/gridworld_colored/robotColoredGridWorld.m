function pomdp = robotColoredGridWorld( rowCount, colCount )
...
obsCount = 16 * 4;
obs_index = ones( 1 , 16 );
obs_map = transpose( reshape( 1:obsCount , 4 , 16 ) );
    ...
    for col = 1:colCount
        for a = 1:actionCount
            ...
            for r = 0:1; for l = 0:1; for d = 0:1; for u = 0:1;
                obsVec = [ r l d u ];
                orig_obs = obsVec * [ 1 2 4 8 ]' + 1;
                obs = obs_map( orig_obs , obs_index( orig_obs ) );
                obs_index( orig_obs ) = mod( obs_index( orig_obs ) , 4 ) + 1;
